...
CodeSystem (``CodeSystem-icd10-danish-extensions.json``, ``content: complete`` by
...
SUPPLEMENT_CANONICAL = "http://hl7.dk/fhir/core/CodeSystem/icd10-danish-extensions"
...
# Identifier for the legacy SKS diagnosis ("D-hierarchy") CodeSystem: the SKS
# diagnosis register published under its own sub-OID. DK Core's profiles bind
# legacy diagnosis codings to this system (Condition.code.coding[SKS-D],
# ServiceRequest.reasonCode.coding[SKS]); kept available so the 3.7.0-style SKS
# coding continues to resolve & display alongside the ICD-10-based model.
SKS_DIAGNOSES_CANONICAL = "urn:oid:1.2.208.176.2.4.12"
...
def build_sks_codesystem(concepts: dict[str, SksConcept], today: str,
    ...
    return {
        ...
        "description": (
            ...
            "icd10-danish-extensions CodeSystem instead. A fragment of the SKS "
...
def build_sks_diagnosis_codesystem(diags: dict[str, "SksDiag"], today: str,
                                   canonical: str, version: str) -> dict:
    """
    Build the legacy SKS diagnosis ("D-hierarchy") CodeSystem from the SKS
    diagnosis register, published under its own sub-OID
    (``urn:oid:1.2.208.176.2.4.12``). Codes are the full SKS diagnosis codes
    *including* the leading "D" (e.g. ``DA022``), with the Danish display text.

    This is the Danish edition of ICD-10 as historically expressed in SKS, kept
    available so the 3.7.0-style legacy bindings in DK Core continue to resolve
    and display. It is a standalone ``content: complete`` code system; the
    ICD-10-based model (plain ICD-10 + ``icd10-danish-extensions`` + the
    ``icd10-da`` supplement) is the modern alternative and is generated
    separately. Hierarchy is modelled as ICD-10 does it: ``hierarchyMeaning:
    is-a`` on a flat concept list with ``parent``/``child`` properties, derived
    positionally from the D-prefixed code structure.
    """
    code_set = set(diags)
    parent_of, children = prefix_hierarchy(code_set)
    ...
    properties = [
        {"code": "status",
         "description": "Whether the SKS diagnosis code is currently active.",
         "type": "code"},
        {"code": "validFrom",
         "description": "Date the SKS code first became valid.",
         "type": "dateTime"},
    ...
    for code in sorted(diags):
        d = diags[code]
        props = [{"code": "status",
                  "valueCode": "active" if d.valid_to >= today else "retired"}]
        vf, vt = to_fhir_date(d.valid_from), to_fhir_date(d.valid_to)
        if vf:
            props.append({"code": "validFrom", "valueDateTime": vf})
        if vt and d.valid_to != OPEN_ENDED_SKS:
            props.append({"code": "validTo", "valueDateTime": vt})
        if code in parent_of:
            props.append({"code": "parent", "valueCode": parent_of[code]})
        for child in sorted(children.get(code, [])):
    ...
    return {
        ...
        "version": version,
        "name": "SKSDiagnoses",
        "title": "SKS Diagnoses - Danish ICD-10 diagnosis register (D-hierarchy)",
        "status": "active",
        "experimental": False,
        "date": dt.date.today().isoformat(),
        "publisher": "HL7 Denmark",
        "jurisdiction": [{"coding": [{
            ...
            "display": "Denmark"}]}],
        "description": (
            "The Danish SKS diagnosis register (the 'D-hierarchy'): the Danish "
            "edition of ICD-10, where each diagnosis code is the ICD-10 code "
            "prefixed with a literal 'D' (e.g. SKS 'DA022' == ICD-10 'A02.2'). "
            "Published under urn:oid:1.2.208.176.2.4.12 and generated from "
            "SKScomplete.txt by scripts/sks_icd10_diff.py. Provided so legacy "
            "SKS diagnosis codings continue to resolve; the modern model is "
            "plain ICD-10 together with the icd10-danish-extensions CodeSystem and "
            "the icd10-da designation supplement."),
        "caseSensitive": True,
        "hierarchyMeaning": "is-a",
        "content": "complete",
        "count": len(concept_list),
...
def build_codesystem(addons: list[dict], icd: Icd10, system: str,
    ...
    cs = {
        ...
        "url": canonical,
        "version": version,
        "name": "Icd10DanishExtensions",
        "title": "Danish extensions and deviations to ICD-10",
        "status": "active",
        "experimental": False,
        "date": dt.date.today().isoformat(),
        "publisher": "HL7 Denmark",
...
def main(argv: list[str]) -> int:
    ...
    p.add_argument("--sks-exclude-registers", default=SKS_EXCLUDE_REGISTERS,
                   help="Comma-separated SKScomplete registers to leave out of "
                        "the SKS CodeSystem.")
    p.add_argument("--sks-diagnoses-canonical", default=SKS_DIAGNOSES_CANONICAL,
                   help="Identifier for the legacy SKS diagnosis (D-hierarchy) "
                        "CodeSystem.")
    p.add_argument("--sks-diagnoses-version", default=None,
                   help="Version for the legacy SKS diagnosis CodeSystem "
    ...
    cs_json = os.path.join(args.out_dir,
                           "CodeSystem-icd10-danish-extensions.json")
    ...
    print("[7/7] Building legacy SKS diagnosis CodeSystem (D-hierarchy) ...")
    sksd_version = args.sks_diagnoses_version or default_version
    sksd_cs = build_sks_diagnosis_codesystem(diags, today,
                                             args.sks_diagnoses_canonical,
                                             sksd_version)
    if source_rev:
        sksd_cs["date"] = source_rev
    sksd_json = os.path.join(args.out_dir, "CodeSystem-sks-diagnoses.json")
    with open(sksd_json, "w", encoding="utf-8") as fh:
        json.dump(sksd_cs, fh, ensure_ascii=False, indent=2)
    print(f"  wrote {sksd_json}  (CodeSystem 'complete', "
          f"{sksd_cs['count']:,} SKS diagnosis codes)")
